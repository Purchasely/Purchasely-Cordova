...
- **`e2e-android.yml` / `e2e-ios.yml`** — Device E2E (Appium + WebdriverIO) against the real backend; run on `workflow_dispatch`, on scoped PRs when they open, and on demand by adding the `run-ci` label (not part of `ci.yml`). See `purchasely/example/e2e/README.md`.
