#!/bin/bash
# Host-side UI driver — polls the device UI for the Purchasely purchase button
# (content-desc contains "action:purchase") and taps its centre.
#
# Usage (run concurrently with the test):
#   bash integration_test/tools/tap_purchase.sh [device_serial]
#
# Exits 0 after a successful tap, 1 on timeout (90 s).
DEV="${1:-emulator-5554}"
