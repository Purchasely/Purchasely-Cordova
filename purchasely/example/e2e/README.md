...
| drawer close tap | `specs/drawer-close-tap.e2e.js` | **hard** on iOS, self-skips on Android | port of React Native T31. A 70% drawer on the placement, closed by its close button and then by a tap on the scrim: `display()` resolves (`closeReason: button` for the button), `PRESENTATION_CLOSED` arrives, and a tap by coordinates on a full-screen probe in the WebView reaches the app. Regression guard for iOS SDK 6.1.2: on 6.1.0 the SDK window stayed over the app, so the probe tap never arrived. Inconclusive (returns, and the iOS runner prints a `::warning::`) when no paywall presents; a missing close button fails |
